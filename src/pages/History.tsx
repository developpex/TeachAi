...
import { Clock, Trash2, Heart, ChevronDown, ChevronUp, HelpCircle, Search, Pencil, Save, X } from 'lucide-react';
  ...
  const loadResponses = async () => {
    try {
      ...
      const initialExpandedState = sorted.reduce((acc, response) => {
        if (response.toolId) {
          acc[response.toolId] = true; // Start expanded
        }
        return acc;
                          ...
                          <div className="flex justify-between items-start mb-4">
                            <div className="flex-1 mr-4">
                              {editingTitle === response.id ? (
                                ...
                                <div className="flex items-center space-x-2">
                                  <h3 className="text-lg font-medium text-primary-dark dark:text-dark-text">
                                    {response.title}
                                  </h3>
                                  <button
                                    onClick={() => {
